...
static int filter_frame(AVFilterLink *inlink, AVFrame *frame)
{
        ...
        if (sd) {
            ...
            for (i = 0; i < sd->size / sizeof(*mvs); i++) {
                ...
                if (s->mv_type) {
                    ...
                    printf("frame_count: %d, frame_type: %c, mv_dst: (%d, %d), mv_src: (%d, %d), mv_type: %c\n",
                        s->frame_count,
    ...
    if (s->last_coded_picture_number != frame->coded_picture_number) {
        s->frame_count++;
    }
...
static av_cold int codecview_init(AVFilterContext *avctx)
{
    CodecViewContext *s = avctx->priv;
    s->last_coded_picture_number = -1; //random initial no.
